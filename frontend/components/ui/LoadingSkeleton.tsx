...
export function SkeletonBox({ className = '' }: { className?: string }) {
  return (
    <div
      className={`bg-[#18222C] animate-pulse rounded-[6px] ${className}`}
    />
  );
}
...
export function TableSkeleton({ rows = 5, cols = 5 }: { rows?: number; cols?: number }) {
  return (
    <div className="bg-[#141C24] border border-[#263442] rounded-[10px] overflow-hidden p-4 space-y-3">
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-[#141C24] border border-[#263442] rounded-[10px] p-5 space-y-3">
