...
export function StatusBadge({ status, className = '', showDot = true }: StatusBadgeProps) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 h-6 px-2.5 rounded-[6px] text-[11px] font-semibold border ${config.bg} ${config.text} ${config.border} ${className} whitespace-nowrap`}
    >
